...
public class JPAPropertyRepositoryImpl implements PropertyRepository {
    ...
    public void removeProperty(int propertyId) throws Exception {
    	//Search property by ID, If property is not null, it means the property has been existed and remove it
    ...
    public List<Property> searchPropertyByBudget(double budget) throws Exception {
    	...
    	Root<Property> eRoot = cQuery.from(Property.class);
    	cQuery.select(eRoot).where(builder.lessThanOrEqualTo(eRoot.get("price").as(Double.class), budget));
        List<Property> lProperties = entityManager.createQuery(cQuery).getResultList(); 
    	return lProperties;
    }
}
